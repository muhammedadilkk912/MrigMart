  ...
  const updateQuantity = async (index, productId, newQuantity) => {
    ...
    try {
      ...
      const response = await axiosInstance.put(
        `/user/updateCartquantity/${productId}/${newQuantity}`
      );
      console.log(response);
        ...
        <div className="h-full bg-white/75 py-12 px-4 sm:px-6 lg:px-8 w-full mx-auto">
          { cartItems.length === 0|| cartItems?.items?.length === 0 ? (
            <div className="flex  sm:pt-20  gap-1 sm:gap-2 items-center justify-center">
              ...
              <Link
                to="/"
                className=" flex  justify-center items-center   sm:gap-2  text-blue-500 hover:text-blue-600 opacity-80  rounded sm:px-1"
              >
                Continue Shopping <FaArrowRight className="pt-1" size={16} />
              </Link>
            </div>
          ) : (
            <>
                ...
                <div className="lg:w-2/3">
                  {cartItems?.items?.map((item, index) => {
                    ...
                    return (
                      <div
                        key={item._id}
                        className="p-4 border-b last:border-0 border-gray-300 hover:bg-gray-100 transition"
                      >
                        <div className="grid grid-cols-1 md:grid-cols-12 gap-4 items-center">
                          ...
                          <div className="md:col-span-3 flex justify-center">
                            <div
                              className={`flex items-center border ${
                                loading === product._id ? " opacity-55" : ""
                              }  rounded-md`}
                            >
                              {/* Minus Button */}
                              <button
                                ...
                                disabled={
                                  item.quantity <= 1 || loading === product._id
                                }
                              >
                                <FaMinus size={12} />
                              </button>
                          ...
                          <div className="md:col-span-2 flex items-center justify-end gap-4">
                            <div className="font-medium">
                              $
                              {(product.discountprice * item.quantity).toFixed(
                                2
                              )}
                            </div>
                            <button
                ...
                <div className="lg:w-1/3">
                  <div className="bg-white rounded-lg shadow-xl p-6 sticky top-4">
                    ...
                    <button
                      ...
                      className="w-full py-3 bg-[#ABBB19] text-white rounded-md hover:bg-[#9aaa10] transition font-medium"
                    >
                      Proceed to Checkout
                    </button>
